#!/usr/bin/env python3
"""
Robustness evaluation untuk Primer conf60 7-class.

Top-3 model (satu per keluarga arsitektur, by val_macro_f1):
  1. Intermediate_TL  (best intermediate fusion, B3 val 0.2917)
  2. LateFusion       (best late fusion, B1 val 0.2887 — scratch unggul TL untuk 7-class)
  3. CNN              (best cnn-only, B1 val 0.2774)
...
Usage:
  CUDA_VISIBLE_DEVICES=1 python scripts/run_eval_7c.py --strategy random --seeds 5
  CUDA_VISIBLE_DEVICES=1 python scripts/run_eval_7c.py --strategy cv5
  CUDA_VISIBLE_DEVICES=1 python scripts/run_eval_7c.py --strategy loso
"""
import argparse
import json
import sys
...
OUT_BASE = PROJECT_ROOT / "models" / "frontonly_conf60" / "7class"
...
# Top-3 models — satu per keluarga arsitektur
TOP3 = {
    "intermediate_tl": {
    ...
    "late_fusion": {
        "arch": "late_fusion",  # CNN scratch + FCNN
        "lr_cnn": LR_SCRATCH,
        "lr_fcnn": LR_SCRATCH,
    },
    "cnn": {
        "builder": lambda: EmotionCNN(num_classes=NUM_CLASSES),
        "arch": "cnn",
        "lr": LR_SCRATCH,
    },
}
...
# ─── Data loading ────────────────────────────────────────────────────────────
def load_all():
    """Load all conf60 data + user_ids, combined across splits. y = 7-class."""
    img = np.concatenate([
...
def train_eval_late_fusion(model_key, train_data, val_data, test_data, ckpt_dir):
    """Train CNN (TL or scratch) + FCNN, grid search w di val."""
    cfg = TOP3[model_key]
    use_tl = (model_key == "late_fusion_tl")
    img_tr, lm_tr, hm_tr, y_tr = train_data
    img_va, lm_va, hm_va, y_va = val_data
    img_te, lm_te, hm_te, y_te = test_data
    ...
    # CNN branch
    cnn = EmotionCNNTransfer(num_classes=NUM_CLASSES).to(device) if use_tl \
          else EmotionCNN(num_classes=NUM_CLASSES).to(device)
    cnn_path = ckpt_dir / f"cnn_{'tl' if use_tl else 'scratch'}_{model_key}.pth"
    cnn_val, cnn_ep = train_single(
        cnn, "cnn",
        build_loader("cnn", img_tr, lm_tr, hm_tr, y_tr, shuffle=True),
        build_loader("cnn", img_va, lm_va, hm_va, y_va),
        criterion, cfg["lr_cnn"], cnn_path,
    )

    # FCNN branch
    fcnn = EmotionFCNN(num_classes=NUM_CLASSES).to(device)
    fcnn_path = ckpt_dir / f"fcnn_{model_key}.pth"
    fcnn_val, fcnn_ep = train_single(
        ...
        criterion, cfg["lr_fcnn"], fcnn_path,
    )
    ...
    for w in np.arange(0.0, 1.05, 0.05):
        ...
        if f1 > best_val_f1:
            best_val_f1, best_w = f1, float(w)

    fused_te = best_w * p_te_cnn + (1 - best_w) * p_te_fcnn
    out = metrics_from_pred(y_te, fused_te.argmax(1))
...
def fit_eval(model_key, train_data, val_data, test_data, ckpt_dir):
    if TOP3[model_key].get("arch") in ("late_fusion", "late_fusion_tl"):
        return train_eval_late_fusion(model_key, train_data, val_data, test_data, ckpt_dir)
...
def run_cv5(D):
    ...
    for model_key in TOP3.keys():
        ...
        if out_path.exists():
            ...
            existing = {"model": model_key, "strategy": "cv5_subject_wise", "per_fold": []}
            done_folds = set()
...
def main():
    ...
    parser.add_argument("--strategy", required=True, choices=["random", "cv5", "loso"])
