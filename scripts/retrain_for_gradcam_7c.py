...
# FACS-28 helper (mirror run_unified_landmark.py + retrain_for_gradcam.py)
FACS_PAIRS = [
...
def load_split(split):
    ...
    # FA landmarks → derive facs_28 for top Intermediate fusion config
...
def main():
    ...
    img_tr, lm_tr, hm_tr, facs28_tr, lm_fa_tr, y_tr = load_split("train")
    ...
    # ── 3. Intermediate_TL — facs_28 FA (top 7c B1 TL variant) ──────────────
    ...
    if ckpt_cnn.exists() and ckpt_fcnn.exists():
        ...
        print(f"\n[late_fusion] training FCNN branch (input=raw_136 FA, dim=136)...")
        fcnn  = EmotionFCNN(input_dim=136, num_classes=NUM_CLASSES).to(device)
        tr_lf = make_loader("fcnn", img_tr, lm_fa_tr, hm_tr, y_tr, shuffle=True)
        va_lf = make_loader("fcnn", img_va, lm_fa_va, hm_va, y_va)
        ...
        p_va_fcnn = get_probs(fcnn, "fcnn", make_loader("fcnn", img_va, lm_fa_va, hm_va, y_va))
        p_te_cnn  = get_probs(cnn,  "cnn",  make_loader("cnn",  img_te, lm_te, hm_te, y_te))
        p_te_fcnn = get_probs(fcnn, "fcnn", make_loader("fcnn", img_te, lm_fa_te, hm_te, y_te))
