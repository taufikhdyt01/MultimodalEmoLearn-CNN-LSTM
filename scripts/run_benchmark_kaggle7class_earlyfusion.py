#!/usr/bin/env python3
"""Early Fusion concat (ResNet-18 TL) di CK+ Kaggle 7-class — Recipe B3.

Dataset: ckplus_kaggle7class — 7 kelas TANPA neutral, DENGAN contempt.
Scheme ini sama dengan Grover 2024, Singh 2025, Gautam 2023.
Labels: 0=anger, 1=contempt, 2=disgust, 3=fear, 4=happy, 5=sadness, 6=surprised

Recipe B3: Adam lr=5e-5, batch 32, 50 epoch, patience 15, internal val 15%,
CrossEntropy + WeightedRandomSampler + synced aug (hflip p=0.5, rotate ±10°,
brightness ±10%, contrast ×0.9-1.1).

Protokol (sample-level, apple-to-apple dengan literatur):
  - randomsplit : StratifiedShuffleSplit 80:20, 5 seed
  - cv10        : StratifiedKFold 10-fold sample-level

Output: models/benchmark/ckplus_samplelevel/ckplus_kaggle7c_{randomsplit,cv10}_earlyfusion_b3.json
Usage:  CUDA_VISIBLE_DEVICES=0 python scripts/run_benchmark_kaggle7class_earlyfusion.py
"""
import json
import os
import sys
...
DATA_DIR = PROJECT_ROOT / "data" / "benchmark" / "ckplus_kaggle7class"
OUT_DIR  = PROJECT_ROOT / "models" / "benchmark" / "ckplus_samplelevel"

EMOTIONS_KAGGLE7 = ["anger", "contempt", "disgust", "fear", "happy", "sadness", "surprised"]
...
def load_all():
    ...
    for sp in ["train", "val", "test"]:
        img.append(np.load(DATA_DIR / f"X_{sp}_images.npy").astype(np.float32))
        hm.append(np.load(DATA_DIR / f"X_{sp}_heatmaps.npy").astype(np.float32))
        y.append(np.load(DATA_DIR / f"y_{sp}.npy").astype(np.int64))
...
def train_eval_fold(img, hm, y, tr_idx, te_idx, nc, fdir):
    ...
    train_model(model, tr_l, vl_l, nn.CrossEntropyLoss(), opt, sch, device, "cnn",
                EPOCHS, PATIENCE, sp)
    model.load_state_dict(torch.load(sp, map_location=device, weights_only=True))
    r = full_evaluation(model, te_l, nn.CrossEntropyLoss(), device, "cnn", EMOTIONS_KAGGLE7)
    os.remove(sp)
    return {"accuracy": float(r["test_accuracy"]), "macro_f1": float(r["test_macro_f1"]),
            "weighted_f1": float(r["test_weighted_f1"])}


def run_protocol(name, folds, img, hm, y, nc=7):
    print(f"\n{'='*64}\n  CK+ Kaggle7c — Early Fusion concat B3 — {name} ({len(folds)} folds)\n{'='*64}",
          flush=True)
    fdir = OUT_DIR / "_tmp_kaggle"; fdir.mkdir(parents=True, exist_ok=True)
    ...
    res = {"model": "EarlyFusion_concat_TL_B3", "protocol": name,
           "dataset_scheme": "kaggle7c (contempt in, neutral out)",
    ...
    save = OUT_DIR / f"ckplus_kaggle7c_{name}_earlyfusion_b3.json"
    json.dump(res, open(save, "w"), indent=2)
    print(f"  {name}: macro_f1={res['macro_f1_mean']:.4f}±{res['macro_f1_std']:.4f}  "
          f"acc={res['accuracy_mean']:.4f}±{res['accuracy_std']:.4f}  -> saved {save.name}",
          flush=True)
    try:
        fdir.rmdir()
    except OSError:
