#!/usr/bin/env python3
"""
Apples-to-apples comparison: MediaPipe vs face-api.js landmarks untuk CNN1D B1.

Train CNN1D B1 (no aug, no class weight) di full dataset_frontonly_conf60
(N=6795 setelah matching 100%), dua skema kelas (3c & 7c), dua source landmark
(mediapipe & faceapi) → total 4 run.
...
def load_data(source: str, num_classes: int):
    """Returns dict with X/y per split. Asserts mask 100% True."""
    out = {}
...
def train_run(data, source: str, num_classes: int):
    ...
    print(f"  val:   {Xva.shape}  counts: {np.bincount(yva, minlength=num_classes).tolist()}")
    print(f"  test:  {Xte.shape}  counts: {np.bincount(yte, minlength=num_classes).tolist()}")
    ...
    return {
        "source": source, "num_classes": num_classes,
...
def main():
    ...
    # Save
    for nc in (3, 7):
        out_dir = PROJECT_ROOT / "models" / "frontonly_conf60" / f"{nc}class" / "CNN1D_geom_compare"
        out_dir.mkdir(parents=True, exist_ok=True)
        out_json = out_dir / "compare_landmark_source.json"
        with open(out_json, "w") as f:
            json.dump({
                "config": f"cnn1d_b1_{nc}c_landmark_source_compare",
    ...
    print("\n" + "=" * 60)
    print("Summary: CNN1D B1, MediaPipe vs face-api.js (full 6795 samples)")
    print("=" * 60)
    for nc in (3, 7):
